module tl_vorticity_advection_kernel_mod_adj
  use kernel_mod, only : kernel_type
  use argument_mod, only : any_discontinuous_space_3, any_space_9, arg_type, &
       cell_column, func_type, gh_basis, gh_diff_basis, gh_field, gh_inc, &
       gh_quadrature_xyoz, gh_read, gh_real
  use constants_mod, only : i_def, r_def
  use fs_continuity_mod, only : w1, w2, WChi
  use cross_product_mod, only : cross_product
  implicit none
  type, public, extends(kernel_type) :: tl_vorticity_advection_kernel_type_adj
  PRIVATE
  TYPE(arg_type) :: meta_args(7) = (/arg_type(GH_FIELD, GH_REAL, GH_READ, W2), &
  ...
  TYPE(func_type) :: meta_funcs(3) = (/func_type(W2, GH_BASIS), &
       func_type(W1, GH_BASIS), &
       func_type(WChi, GH_BASIS, GH_DIFF_BASIS)/)
  INTEGER :: operates_on = CELL_COLUMN
  INTEGER :: gh_shape = GH_QUADRATURE_XYoZ
  CONTAINS
  PROCEDURE, NOPASS :: tl_vorticity_advection_code_adj
END TYPE
  public

  public :: tl_vorticity_advection_code_adj

  contains
    subroutine tl_vorticity_advection_code_adj(nlayers, r_u, wind, vorticity, &
         ls_wind, ls_vorticity, chi_1, chi_2, chi_3, panel_id, ndf_w2, &
         undf_w2, map_w2, w2_basis, ndf_w1, undf_w1, map_w1, w1_basis, &
         ndf_chi, undf_chi, map_chi, chi_basis, chi_diff_basis, ndf_pid, &
         undf_pid, map_pid, nqp_h, nqp_v, wqp_h, wqp_v)
    ...
    do k = nlayers - 1, 0, -1
      ...
      do qp2 = nqp_v, 1, -1
        do qp1 = nqp_h, 1, -1
           call pointwise_coordinate_jacobian(ndf_chi, chi_1_e, chi_2_e, &
                chi_3_e, ipanel, chi_basis(:,:,qp1,qp2), &
          ...
          do df = 1, ndf_w1, 1
             ls_vorticity_at_quad(:) = ls_vorticity_at_quad(:) + &
                  ls_vorticity(map_w1(df) + k) * w1_basis(:,df,qp1,qp2)
          enddo
          ...
          do i_1 = 1, 3, 1
            mul1(i_1) = 0.0_r_def
            do j_1 = 1, 3, 1
              mul1(i_1) = mul1(i_1) + jac(i_1,j_1) * ls_vorticity_at_quad(j_1)
            enddo
          enddo
          ...
          do df = ndf_w2, 1, -1
            ...
            do i = 3, 1, -1
              vorticity_term(i) = vorticity_term(i) + w2_basis(i,df,qp1,qp2) * res_dot_product
            enddo
            res_dot_product = 0.0
          enddo
          ...
          do idx_2 = UBOUND(j_vorticity, 1), LBOUND(j_vorticity, 1), -1
            mul2(idx_2) = mul2(idx_2) + wqp_h(qp1) * wqp_v(qp2) * j_vorticity(idx_2)
          ...
          do df = ndf_w2, 1, -1
            do idx_2 = UBOUND(u_at_quad, 1), LBOUND(u_at_quad, 1), -1
              wind(map_w2(df) + k) = wind(map_w2(df) + k) + u_at_quad(idx_2) * w2_basis(idx_2,df,qp1,qp2)
            enddo
          enddo
          ...
          do df = ndf_w1, 1, -1
            do idx_1 = UBOUND(vorticity_at_quad, 1), LBOUND(vorticity_at_quad, 1), -1
               vorticity(map_w1(df) + k) = vorticity(map_w1(df) + k) + &
                    vorticity_at_quad(idx_1) * w1_basis(idx_1,df,qp1,qp2)
            enddo
          enddo
  ...
  end subroutine tl_vorticity_advection_code_adj

end module tl_vorticity_advection_kernel_mod_adj
