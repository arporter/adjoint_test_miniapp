module tl_kinetic_energy_gradient_kernel_mod_adj
  use argument_mod, only : any_discontinuous_space_3, any_space_9, &
  ...
  implicit none
  type, public, extends(kernel_type) :: tl_kinetic_energy_gradient_kernel_type_adj
  PRIVATE
  ...
  CONTAINS
  PROCEDURE, NOPASS :: tl_kinetic_energy_gradient_code_adj
END TYPE
  private

  public :: tl_kinetic_energy_gradient_code_adj

  contains
    subroutine tl_kinetic_energy_gradient_code_adj( &
         nlayers, r_u, u, ls_u, chi_1, chi_2, chi_3, panel_id, &
    ...
    do k = nlayers - 1, 0, -1
      ...
      enddo
      call coordinate_jacobian(ndf_chi, nqp_h, nqp_v, chi_1_e, chi_2_e, &
           chi_3_e, ipanel, chi_basis, chi_diff_basis, jac, dj)
      do df = 1, ndf_w2, 1
      ...
      do qp2 = nqp_v, 1, -1
        do qp1 = nqp_h, 1, -1
          ...
          do i = 1, 3, 1
            mul1(i) = 0.0
            do j = 1, 3, 1
              mul1(i) = mul1(i) + jac(i,j,qp1,qp2) * ls_u_at_quad(j)
            enddo
          enddo
          do df = ndf_w2, 1, -1
            dv = w2_diff_basis(1,df,qp1,qp2)
            ke_at_quad = ke_at_quad + wqp_h(qp1) * wqp_v(qp2) * dv * ru_e(df)
          enddo
  ...
  end subroutine tl_kinetic_energy_gradient_code_adj

end module tl_kinetic_energy_gradient_kernel_mod_adj
