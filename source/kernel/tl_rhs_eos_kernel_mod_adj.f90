module tl_rhs_eos_kernel_mod_adj
  use argument_mod, only : any_discontinuous_space_3, any_space_9, arg_type, &
       cell_column, func_type, gh_basis, gh_diff_basis, gh_field, &
       gh_quadrature_xyoz, gh_read, gh_real, gh_scalar, gh_write, gh_readwrite
  use constants_mod, only : i_def, r_def
  use fs_continuity_mod, only : w3, wtheta
  use kernel_mod, only : kernel_type
  implicit none
  type, public, extends(kernel_type) :: tl_rhs_eos_kernel_type_adj
  PRIVATE
  ...
  CONTAINS
  PROCEDURE, NOPASS :: tl_rhs_eos_code_adj
END TYPE
  private

  public :: tl_rhs_eos_code_adj

  contains
    subroutine tl_rhs_eos_code_adj(nlayers, rhs_eos, exner, rho, theta, &
         moist_dyn_gas, ls_exner, ls_rho, ls_theta, ls_moist_dyn_gas, &
         chi1, chi2, chi3, panel_id, kappa, rd, p_zero, &
         ndf_w3, undf_w3, map_w3, w3_basis, ndf_wt, undf_wt, map_wt, &
         wt_basis, ndf_chi, undf_chi, map_chi, chi_basis, chi_diff_basis, &
    ...
    do k = nlayers - 1, 0, -1
      ...
      enddo
      call coordinate_jacobian(ndf_chi, nqp_h, nqp_v, chi1_e, chi2_e, chi3_e, ipanel, chi_basis, chi_diff_basis, jac, dj)
      do df = 1, ndf_wt, 1
        ls_theta_vd_e(df) = ls_theta(map_wt(df) + k) * ls_moist_dyn_gas(map_wt(df) + k)
      enddo
      do df = 1, ndf_w3, 1
      ...
      do qp2 = nqp_v, 1, -1
        do qp1 = nqp_h, 1, -1
          ...
          enddo
          ls_eos = 0.0_r_def - p0_over_rd * ls_exner_quad ** onemk_over_k / (ls_rho_quad * ls_theta_vd_quad)
          do df = ndf_w3, 1, -1
            eos = eos - w3_basis(1,df,qp1,qp2) * rhs_eos(map_w3(df) + k)
          enddo
      ...
      enddo
      do df = ndf_wt, 1, -1
        moist_dyn_gas(map_wt(df) + k) = moist_dyn_gas(map_wt(df) + k) + ls_theta(map_wt(df) + k) * theta_vd_e(df)
        theta(map_wt(df) + k) = theta(map_wt(df) + k) + theta_vd_e(df) * ls_moist_dyn_gas(map_wt(df) + k)
        theta_vd_e(df) = 0.0
      enddo
    enddo

  end subroutine tl_rhs_eos_code_adj

end module tl_rhs_eos_kernel_mod_adj
